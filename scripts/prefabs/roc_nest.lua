...
-- I think it would be cool to use this new DST functionality to make the nest feel better. Changing its step sound and making it a little hard to walk over those little twigs and debris and stuff.
local function OnUpdate(inst, x, y, z)
    for i, v in ipairs(TheSim:FindEntities(x, y, z, 7, SLOWDOWN_MUST_TAGS, SLOWDOWN_CANT_TAGS)) do
        if v.components.locomotor ~= nil then
            v.components.locomotor:PushTempGroundSpeedMultiplier(1, WORLD_TILES.GRASS)
        end
    end
end
...
--[[
    ...
    -- inst.components.unevenground.radius = 7 -- Testing an alternative thing
	
	-- Just condition this behind an 'if' or something if we want to put this in configuration
