...
local function recallmarker_ondropped(inst)
	if inst._owner ~= nil then
		...
		inst:RemoveEventCallback("ms_changeroom", inst._changeroom_handler, inst._owner)
		inst._owner = nil
	end
		...
		if owner ~= nil and not owner:HasTag("playerghost") then
			...
			if owner ~= nil and x ~= nil then
				...
				if inst._owner == nil then
					...
					inst:ListenForEvent("ms_changeroom", inst._changeroom_handler, owner)
				end
			end
		end
	end
...
local function recallmarker_roomchange(inst)
	print("DS - Pocketwatch Common - RoomChange triggered, update the markers")
	recallmarker_create(inst)
end
...
local function MakeRecallMarkable(inst)
	...
	-- inst:ListenForEvent("ms_changeroom", recallmarker_roomchange) -- For interior updating stuff
	-- I can't believe this. Replacing the entire file just to add this line. How yucky.

	inst._respawnedfromghost_handler = function() recallmarker_create(inst) end
	inst._changeroom_handler = function() recallmarker_roomchange(inst) end
	print("DS - Pocketwatch Common - Added room event handler")
